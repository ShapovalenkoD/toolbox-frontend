# Структура проекта - краткая версия

> Основной документ для карты каталогов, размещения, публичного API и зависимостей. Контракты слоёв и примеры: [полная версия](ProjectStructure.full.md). [Роли документов](../README.md#документация).

## Корень проекта (рядом с `package.json`)

- [`assets/`](ProjectStructure.full.md#assets---статические-ресурсы) - статические ресурсы. Расположение `assets/`/`public/` и способ обработки определяются сборщиком, см. полную версию.
- [`config/`](ProjectStructure.full.md#config---конфигурация-приложения) - типизированный доступ к env и feature flags. Место `.env` определяется инструментом проекта.
- [`docs/`](ProjectStructure.full.md#docs---документация-проекта) - документация проекта.
- `src/` - прикладной исходный код; корневой `config/` — оговорённое исключение для конфигурации. Всё ниже — внутри `src/`.

## Внутри `src/`

- [`app/`](ProjectStructure.full.md#app---точка-входа-приложения) - точка входа и композиция провайдеров. **Не содержит** конфигурацию store.
- `pages/` — модули страниц с локальными компонентами, интерфейсами и преобразованиями. **Не создаётся** при file-based routing (Next.js App Router, TanStack Router) — см. [Routing](ProjectStructure.full.md#routing).
- `routes/` — конфигурация маршрутов и guards, когда маршрутизация нужна. При обязательной файловой структуре роутера/framework (например, Next.js App Router, TanStack Router) следуем ей полностью, включая размещение компонента экрана — `pages/` при этом не создаём параллельно.
- [`components/`](ProjectStructure.full.md#components---компоненты-приложения) - общая папка для всех компонентов.
  - `business/` - компоненты прикладных сценариев и специфичной для проекта композиции
    - `layouts/` - переиспользуемое расположение областей приложения; без управления данными и прикладными операциями
    - `patterns/` - настраиваемые сценарии, получающие данные, правила и операции через props: DataGrid объединяет Table и Pagination и управляет загрузкой
    - `features/` - конкретные бизнес-сценарии; могут подключать services и store
  - `ui/` - общая папка для UI-кита
    - `foundation/` - глобальные стили по отдельным файлам: цвета, типографика, reset, токены, базовые правила
    - `atoms/` - глобальные CSS-классы HTML-примитивов (button, button-size-lg, button-variant-primary), stories; без production React-компонентов
    - `molecules/` - небольшие UI-композиции: TextField, LoadingButton
    - `organisms/` - сложный UI: Table, Pagination с вычислением окна страниц и переходов; не управляют внешними операциями
- [`constants/`](ProjectStructure.full.md#constants---глобальные-константы-приложения) - глобальные константы проекта (regex, роуты, общие значения).
- [`lib/`](ProjectStructure.full.md#lib---утилиты-и-вспомогательные-функции) - общие утилиты и библиотеки.
  - `hooks/` - хуки проекта
  - `ts/` - утилиты-дженерики для TypeScript
  - `business/` - утилиты, зависящие от бизнес-логики проекта
  - `utils/` - чистые утилиты без контекста проекта
- [`mocks/`](ProjectStructure.full.md#mocks---моковые-данные-для-тестов-и-разработки) - моковые данные для тестов и разработки.
  - `service/` - моки API-сервисов
  - `components/` - моки для пропсов компонентов
- [`services/`](ProjectStructure.full.md#services---работа-с-side-эффектами) - внешние взаимодействия: API, localStorage, sessionStorage, cookies, обмен между вкладками. Принимают/возвращают DTO; могут проверять их схему.
  - `api/` - HTTP-клиент, endpoint-функции, при необходимости React Query/SWR-интеграция по доменам; без преобразований под UI
  - `localStorage/` - методы для работы с localStorage
  - `sessionStorage/` - методы для работы с sessionStorage
  - `cookies/` - методы для работы с cookies
  - `channels/` - обмен сообщениями между вкладками, если требуется
  - `types/` - общие типы для работы с services
- [`store/`](ProjectStructure.full.md#store---клиентское-состояние) - внутреннее состояние приложения. Может принимать модели, собранные страницей/feature из внешних данных; не знает о DTO и не импортирует services.

## Локальность и поток данных

Страница — модуль. Собственные companion-файлы экрана (`PageName.interface.ts`, `.hooks.ts`, `.constants.ts`, `.schema.ts`) лежат рядом с ним, а самостоятельные локальные модули — в служебных папках с префиксом `-`: `-components/`, `-utils/`, `-hooks/`. Они остаются локальными, пока принадлежат этой странице.

Префикс `-` выбран как единый для любого способа маршрутизации: TanStack Router исключает такие файлы и папки из route tree, Next.js App Router не делает маршрутом папку без `page`/`route`, а для config-based routing это обычная папка. Варианты `_components/` (в TanStack Router — pathless-маршрут) и `(children)/` (route group, файлы внутри становятся маршрутами) не используются.

`services → DTO → страница/бизнес-компонент → локальное представление → UI и/или store` — это поток данных, не схема импортов. При отправке локальное представление преобразуется в request DTO и передаётся сервису.

Конвертеры `{entity}FromDto.ts` / `{entity}ToDto.ts` принадлежат потребителю. Совпадение преобразований разных страниц допустимо. Общим становится целый бизнес-сценарий вместе с его преобразованиями, а не отдельный глобальный конвертер. Основной контракт: [DTO vs внутренние модели](ProjectStructure.full.md#dto-vs-внутренние-модели).

Критерии выбора UI и бизнес-компонента: [organisms, patterns, features](ProjectStructure.full.md#граница-organisms-patterns-и-features).

---

# Правила организации файлов

## Правило: каждый домен = отдельная папка

Категорически запрещено складывать файлы разных доменов в одну папку. Каждая бизнес-сущность (auth, user, product, order и т.д.) получает **свою подпапку**.

**Неправильно** — всё в одной папке:

```
services/api/
  ├── login.ts
  ├── logout.ts
  ├── getUser.ts
  ├── getProduct.ts
  └── createOrder.ts
```

**Правильно** — каждый домен в отдельной папке:

```
services/api/
  ├── auth/
  │   ├── login.ts
  │   └── logout.ts
  ├── user/
  │   └── getUser.ts
  ├── product/
  │   └── getProduct.ts
  └── order/
      └── createOrder.ts
```

Это правило применяется **везде**: `services/api/`, `store/`, `components/business/features/`, `mocks/service/`. В схемах выше показано только разделение по доменам; индексы создаются по правилам публичного API ниже.

## File Naming Convention

Имена файлов и соглашения о companion-файлах определены в [Code Style Guide](CODE_STYLE_GUIDE.md#file-naming-convention). Публичный API и размещение модулей описаны ниже.

### Дерево публичных экспортов

Индекс создаётся в каждой папке с публичными экспортами, включая группирующие каталоги; пустые индексы не создаём.

Индексы образуют дерево от конечных модулей до корня слоя, включая группирующие папки. Каждый уровень выбирает, какие файлы и дочерние модули предоставляет наружу. Для доступа через корневой индекс сущность должна быть реэкспортирована на каждом промежуточном уровне.

```text
components/index.ts
  → business/index.ts
    → patterns/index.ts
      → dataGrid/index.ts
        → DataGrid, DataGridProps
  → ui/index.ts
    → organisms/index.ts
      → pagination/index.ts
        → Pagination, PaginationProps
```

- Индексы используют `export *` для исполняемого кода и `export type *` для файлов интерфейсов. Публичность задаётся выбором реэкспортируемых файлов и дочерних модулей: внутренний файл в индекс не включается. Если файл содержит и публичные, и внутренние сущности, внутренние выносятся в отдельный файл либо для этого файла используется именованный реэкспорт.

  ```ts
  // components/ui/molecules/textField/index.ts
  export * from "./TextField";
  export type * from "./TextField.interface";

  // components/ui/molecules/index.ts
  export * from "./loadingButton";
  export * from "./textField";
  ```

- Совпадение имён в двух `export *` одного индекса является ошибкой TypeScript: сущность переименовывают, а не скрывают выборочным реэкспортом.
- Публичными могут быть компоненты, функции, хуки, типы, константы и стили. Внутренние сущности, тесты и stories в публичную цепочку не включаются.
- Внешний потребитель использует индекс разрешённого модуля; прямые импорты его внутренних файлов запрещены соглашением. Сам `index.ts` технически не делает соседние файлы приватными.
- Внутри модуля используются прямые относительные импорты файлов собственной реализации. Для отдельного дочернего/соседнего модуля используется его индекс. Реализация не импортирует собственный индекс или индекс предка.
- Публичность относительна границе: `-utils/cardFromDto/index.ts` открывает конвертер странице, но страница не реэкспортирует его в свой внешний API. Группирующий `-utils/index.ts` может собирать такие экспорты внутри страницы; это не делает их общими для других страниц.
- Импорт из общего barrel допустим, только если все его исполняемые реэкспорты разрешены потребителю. Например, UI использует `lib/utils` или конкретный допустимый модуль, а не корневой `lib`, собирающий также бизнес-утилиты. Для type-only импорта проверяется происхождение выбранного типа.
- Корневой barrel не отменяет архитектурные границы: UI не импортирует из `components/index.ts`, объединяющего UI и business. Внутри дерева используются индексы разрешённых нижележащих/соседних модулей без циклов. Код app может использовать корневой components API.
- CSS Modules могут предоставлять именованный экспорт объекта классов через индекс, например `export { default as cardStyles } from "./card.module.css";` (здесь именованный реэкспорт обязателен: `export *` не переносит default); atoms используют обычный CSS. Это адаптация контракта CSS Modules, а не default export прикладного модуля.
- Глобальный CSS подключается как stylesheet ради применения стилей; искусственный TypeScript-экспорт для него не нужен. CSS-only папка без публичного JS/TS API не требует пустого `index.ts`.

### Модуль утилиты

Каждую самостоятельную утилиту, включая локальный DTO-конвертер, оформляем собственной папкой с реализацией и индексом. Индекс локальной утилиты доступен внутри модуля-владельца, а не другим страницам. Небольшие внутренние вспомогательные функции внутри реализации не обязаны становиться отдельными модулями. Собственные интерфейсы находятся рядом, если нужны:

```text
formatPrice/
  index.ts
  formatPrice.ts
  formatPrice.interface.ts  # только при наличии собственных интерфейсов
```

Наличие интерфейса в отдельном файле не делает его публичным: наружу экспортируются только типы публичного контракта. Пустые companion-файлы не создаём.

**Пример структуры компонента**:

```
productCard/
├── index.ts                     # barrel-экспорт
├── ProductCard.tsx              # компонент
├── ProductCard.hooks.ts         # хуки компонента
├── ProductCard.interface.ts     # интерфейсы пропсов и внутренние типы
├── ProductCard.test.tsx         # тесты
├── ProductCard.stories.tsx      # storybook
└── productCard.module.css       # стили (формат зависит от проекта: CSS Modules, styled-components и т.д.)
```

---

# Dependency rules — карта допустимых зависимостей

`A → B` означает «A может импортировать B». Ниже перечислены разрешения между архитектурными зонами; обратное направление из них не следует. Неперечисленные межслойные зависимости запрещены; локальные файлы одного модуля могут зависеть друг от друга без циклов.

| Кто | Допустимые зависимости |
|---|---|
| app / routes | pages, components, services, store, lib, constants, config |
| pages | components, services, store, lib, constants, config |
| business/features | UI, patterns, layouts, services, store, lib, constants, config |
| business/patterns | UI, layouts, lib, constants; DTO-типы services только для собственного входного контракта/конвертеров |
| business/layouts | UI, lib/utils, constants |
| UI | Собственные UI-модули, lib/utils, общие UI-хуки lib/hooks и type-only утилиты lib/ts; без бизнес-контекста |
| services | Другие сервисные модули, lib, constants, config |
| store | Собственные модули store, lib, constants; без services и DTO |
| lib | Другие lib-модули, constants; без services, store, components, app, routes и pages |
| constants | Без зависимостей от остальных зон |
| config | Без зависимостей от app, routes, pages, components, services и store |

Внешние библиотеки допустимы в соответствии с ответственностью слоя. Внутренние зависимости не должны образовывать циклы. UI-композиция направлена от organisms к molecules/atoms/foundation, от molecules к atoms/foundation и от atoms к foundation; нижние уровни не импортируют верхние. Композиция модулей одного UI-уровня допускается без циклов, если не нарушает их ответственность.

**Обязательные ограничения:**
- Services и store не импортируют друг друга, включая `import type`.
- Patterns не импортируют исполняемые services/store, в том числе через промежуточные хуки. Переданные операции через props разрешены.
- UI не импортирует бизнес-модули, services, store или DTO-типы и не получает операции доступа к данным как зависимости.
- Страница не импортирует локальные конвертеры другой страницы; общий сценарий выделяется целиком в business.
- Production-код не импортирует mocks. Тесты, stories и отдельная точка входа демо могут импортировать общие моки; их настройки изоляции описаны в [разделе mocks](ProjectStructure.full.md#mocks---моковые-данные-для-тестов-и-разработки).
- Ограничения распространяются на относительные пути, алиасы и реэкспорты. Исключение для DTO-типов patterns не разрешает runtime-импорт.

Контракты ответственности и примеры: [полная версия](ProjectStructure.full.md). Правила imports должны быть отражены в конфигурации линтера проекта; одного наличия таблицы недостаточно.
