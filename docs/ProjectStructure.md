...
- [`app`](#app---точка-входа-приложения) - точка входа в приложение: провайдеры, роутинг, страницы. **Не содержит** конфигурацию store.
- [`assets`](#assets---статические-ресурсы) - статические ресурсы (изображения, шрифты, SVG-иконки).
- [`components`](#components---компоненты-приложения) - общая папка для всех компонентов.
...
- [`config`](#config---конфигурация-приложения) - конфигурация приложения (env-переменные, feature flags).
- [`constants`](#constants---глобальные-константы-приложения) - глобальные константы проекта (regex, роуты, общие значения).
- [`lib`](#lib---утилиты-и-вспомогательные-функции) - общие утилиты и библиотеки.
  - `hooks` - хуки проекта
...
- [`mocks`](#mocks---моковые-данные-для-тестов-и-разработки) - моковые данные для тестов и разработки.
  - `service` - моки API-сервисов
  - `components` - моки для пропсов компонентов
- [`services`](#services---работа-с-side-эффектами) - сервисы для работы с side-effects (API, localStorage, cookie).
...
- [`store`](#store---глобальный-стейт-менеджмент) - глобальный store. **Вся** конфигурация (rootReducer, middleware, типы) живёт здесь.
