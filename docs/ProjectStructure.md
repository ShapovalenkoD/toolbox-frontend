...
- Barrel-экспорт (`index.ts`) в каждой папке с публичными экспортами, включая группирующие каталоги; пустые индексы не создаём
...
- Если папка содержит несколько компонентов — экспортирует только выбранные публичные компоненты; остальные остаются внутренними.
- Пример:
  ```ts
