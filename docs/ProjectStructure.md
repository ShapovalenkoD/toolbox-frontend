...
- Именованный экспорт в прикладном коде; обязательные default exports фреймворков и инструментов допускаются — см. [правила экспорта](CODE_STYLE_GUIDE.md#экспорт).
- Barrel-экспорт (`index.ts`) в каждой папке
