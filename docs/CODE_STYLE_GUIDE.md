...
В React-профиле базовый инструмент — Biome. Исполняемый ориентир по образцу cs2_frontend: [biome.json](../examples/react-reference/biome.json) и дополняющий [eslint.config.js](../examples/react-reference/eslint.config.js). Настройки старого проекта переносятся осмысленно, с проверкой версии инструмента.
...
В примере Biome и ESLint запускаются совместно командами format / format:check. Другой набор инструментов в существующем проекте не заменяем автоматически. Условия использования Tailwind и сортировки его классов находятся в [React-профиле](ReactPreferences.md#tailwind).
...
Функции утилит, включая локальные DTO-конвертеры страницы, оформляем с блочным телом и явным `return`, даже при возврате одного объекта. Вычисления и проверки отделяем от возврата пустой строкой. В React-примере это закреплено правилом ESLint `arrow-body-style` для каталогов утилит. Для generic-перегрузок допустимы именованные function declarations.
...
Ниже — соответствия правилам ESLint для проектов с этим инструментом. В React-примере используется Biome с дополнениями ESLint из раздела форматирования; таблица не означает, что каждый перечисленный ESLint-плагин установлен. Ссылки на правила не заменяют конфигурацию.
...
Для условного объединения CSS-классов используем clsx, а не массивы с filter(Boolean).join(" "). Если выбран Tailwind, потенциальные конфликты utilities разрешаем через tailwind-merge; при сложных условных аргументах допустимо twMerge(clsx(...)).
